...
public class Fish : MonoBehaviour
{
    ...
    [SerializeField] private float turnSpeed = 0.4f; // 降低转向速度，更平滑
    ...
    [SerializeField] private float maxTiltAngle = 5f; // 降低倾斜角度限制
    ...
    private void HandleMovement()
    {
        ...
        // 旋轉 - 修正：保持鱼基本直立，防止平躺漂浮
        if (currentVelocity.sqrMagnitude > 0.0001f)
        {
            // 计算目标旋转：主要基于水平方向
            Vector3 lookDir = currentVelocity.normalized;
            lookDir.y = Mathf.Clamp(lookDir.y, -0.3f, 0.3f); // 限制垂直分量，防止过度倾斜
            
            if (lookDir.sqrMagnitude > 0.0001f)
            {
                // 基础旋转：鱼朝向前进方向
                Quaternion targetRot = Quaternion.LookRotation(lookDir) * Quaternion.Euler(forwardOffset);
                ...
                float banking = Mathf.Clamp(turnAngle * 0.2f, -maxTiltAngle, maxTiltAngle);
                
                // 微妙的俯仰：上游/下游时轻微抬头/低头
                float pitch = Mathf.Clamp(currentVelocity.y * 30f * worldScale, -maxTiltAngle * 0.6f, maxTiltAngle * 0.6f);
                
                // 组合旋转：应用微小的 pitch 和 roll
                Quaternion tiltRot = Quaternion.Euler(pitch, 0, banking);
                
                // 平滑过渡到目标旋转
                transform.rotation = Quaternion.Slerp(transform.rotation, targetRot * tiltRot, dt * turnSpeed * 2f);
            }
        }
    }
